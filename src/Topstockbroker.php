...
use ReflectionClass;
use Stackadroit\Topstockbroker\Modules\AbstractModule;

use function apply_filters;
use function do_action;

class Topstockbroker
{
    ...
    /**
     * Create an instance of Topstockbroker.
     *
     * @param string[]|object[]
     * @return void
     */
    ...
    public function __invoke()
    {
        ...
        $modules = array_unique(array_filter(
            apply_filters('topstockbroker/modules', (array) $this->modules)
        ));
        ...
        foreach ($modules as $module) {
            if (is_string($module)) {
                /** @var \Stackadroit\Topstockbroker\Modules\AbstractModule */
                $module = new $module();
            }
    ...
    protected function features($features = [])
    {
        ...
        foreach ((array) $features as $module => $options) {
            // add_theme_support('topstockbroker', ['module'])
            if (is_int($module)) {
                $module = $options;
                $options = ['enabled' => true];
            }

            // add_theme_support('topstockbroker', ['module' => true])
            if (is_bool($options)) {
                $options = ['enabled' => $options];
            }

            // add_theme_support('topstockbroker', ['module' => 'some-option'])
            if (!is_array($options)) {
            ...
            // add_theme_support('topstockbroker', ['module' => ['option' => 'value']])
